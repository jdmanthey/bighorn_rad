...
calc_diff <- TRUE				# calculate pairwise FST and DXY for all pairwise comps. of pops with 
									# N >= calc_diff_minN
calc_FIS <- TRUE					# calculate FIS for all pops with N >= calc_FIS_minN
...
vcf_type <- "chromosomes" # two options, "windows" or "chromosomes"
...
filename_simple <- strsplit(args[1], "stats_")[[1]][2] 
if(vcf_type == "chromosomes") {
	...
	chromosome_info <- c(strsplit(filename_simple, "__")[[1]][1], 
                    ...
                    as.numeric(strsplit(strsplit(filename_simple, "__")[[1]][3])))
}
